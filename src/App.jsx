    ...
    <div className="min-h-screen bg-white pb-16 md:pb-0" style={{ fontFamily: 'var(--font-body)' }}>
      ...
      {page === 'cart' && <div className="max-w-xl mx-auto px-4 sm:px-6 py-6">
        ...
        {cart.length === 0 ? <div className="text-center py-14"><p className="text-gray-300 text-sm mb-3">Your cart is empty</p><button onClick={() => go('shop','/shop')} className="h-9 px-5 bg-[var(--color-brand)] text-white rounded-lg text-xs font-semibold">Browse Products</button></div> : <>
          ...
          <button onClick={() => { setCheckoutStep(1); setPage('checkout'); window.location.hash = '/checkout' }} className="w-full h-11 bg-[var(--color-brand)] text-white rounded-lg text-sm font-semibold hover:bg-black transition">Checkout · {money(ct)}</button>
          <a href={`https://wa.me/${WA}?text=${encodeURIComponent(`Hi, I'd like to order:\n\n${cart.map(c => `${c.qty}x ${c.name} — ${money(c.price * c.qty)}`).join('\n')}\n\nTotal: ${money(ct)}`)}`} target="_blank" className="w-full h-10 mt-2 bg-gray-50 text-gray-600 rounded-lg text-xs font-semibold hover:bg-gray-100 transition flex items-center justify-center gap-2 border border-gray-100">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="#25d366"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
            Order via WhatsApp
          </a>
        </>}
      ...
      {page === 'checkout' && <div className="max-w-md mx-auto px-4 sm:px-6 py-6">
          <button onClick={() => checkoutStep > 1 ? setCheckoutStep(checkoutStep - 1) : go('cart','/cart')} className="text-[11px] text-gray-400 hover:text-gray-600 mb-5 inline-flex items-center gap-1">&larr; {checkoutStep > 1 ? 'Back' : 'Cart'}</button>
            ...
            {[
              ...
              <div key={s.n} className="flex items-center flex-1">
                <div className="flex flex-col items-center">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 ${checkoutStep > s.n ? 'bg-green-500 text-white' : checkoutStep === s.n ? 'bg-[var(--color-brand)] text-white shadow-lg shadow-black/10' : 'bg-gray-100 text-gray-300'}`}>
                    {checkoutStep > s.n ? <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><path d="M20 6L9 17l-5-5"/></svg> : s.icon}
                  </div>
                  <span className={`text-[10px] mt-1.5 font-semibold ${checkoutStep >= s.n ? 'text-gray-700' : 'text-gray-300'}`}>{s.label}</span>
                </div>
                {i < 2 && <div className={`flex-1 h-0.5 mx-2 mb-5 rounded transition-all duration-500 ${checkoutStep > s.n ? 'bg-green-400' : 'bg-gray-100'}`} />}
              </div>
            ))}
          </div>

          {/* Step 1: Review Order */}
          {checkoutStep === 1 && <div style={{ animation: 'fadeIn 0.3s ease' }}>
            ...
            <button onClick={() => setCheckoutStep(2)} className="w-full h-12 bg-[var(--color-brand)] text-white rounded-xl text-sm font-bold hover:bg-black transition active:scale-[0.98]">Continue to Details</button>
          </div>}

          {/* Step 2: Your Details */}
          {checkoutStep === 2 && <div style={{ animation: 'fadeIn 0.3s ease' }}>
            ...
            <button onClick={() => setCheckoutStep(3)} disabled={!custName.trim() || !custPhone.trim() || (fulfillment === 'delivery' && !custAddress.trim())} className="w-full h-12 bg-[var(--color-brand)] text-white rounded-xl text-sm font-bold hover:bg-black transition active:scale-[0.98] disabled:opacity-30">Continue to Confirm</button>
          </div>}

          {/* Step 3: Confirm & Pay */}
          {checkoutStep === 3 && <div style={{ animation: 'fadeIn 0.3s ease' }}>
